...
// ---- Connections: each instrument input takes one parameter (or none) -----
// null = not connected (silent). A parameter can feed multiple instruments.
...
// ---- History graph ------------------------------------------------------
// A scrolling time-series of the raw ball parameters (CC 0-6, normalized 0..1)
// plus the derived roll speed. ~10s window at 60fps.
const HIST_MAX = 600;
const HIST_SERIES = [
  { label: "roll speed", color: "#28e0a0", width: 2.5, get: () => rollSpeed },
  { label: "roll rate", color: "#00e5ff", width: 1.5, get: () => rollRaw },
  { label: "energy", color: "#ffd166", width: 1.5, get: () => lastMotion },
  { label: "tap", color: "#ff5db1", width: 1.5, get: () => tapEnv },
  ...[0, 1, 2, 3, 4, 5, 6].map((n) => ({
    label: `CC${n}`,
    color: `hsl(${20 + n * 45} 75% 62%)`,
    width: 1,
    get: () => (cc[n] ?? 0) / 127,
  })),
];
const histBuf = HIST_SERIES.map(() => []);
let histCtx = null;

function setupHistory() {
  histCtx = els.histCanvas.getContext("2d");
  els.histLegend.innerHTML = HIST_SERIES.map(
    (s) => `<span class="item"><span class="sw" style="background:${s.color}"></span>${s.label}</span>`
  ).join("");
}
...
function drawHistory() {
  ...
  const cssW = els.histCanvas.clientWidth;
  const cssH = els.histCanvas.clientHeight;
  if (els.histCanvas.width !== Math.round(cssW * dpr) ||
      els.histCanvas.height !== Math.round(cssH * dpr)) {
    els.histCanvas.width = Math.round(cssW * dpr);
    els.histCanvas.height = Math.round(cssH * dpr);
  }
  const ctx = histCtx;
  const W = els.histCanvas.width;
  const H = els.histCanvas.height;
  ...
  HIST_SERIES.forEach((s, si) => {
    const buf = histBuf[si];
    ...
    for (let i = 0; i < buf.length; i++) {
      ...
      const y = H - 1 - Math.max(0, Math.min(1, buf[i])) * (H - 2);
...
function sampleHistory() {
  for (let i = 0; i < HIST_SERIES.length; i++) {
    const buf = histBuf[i];
    buf.push(HIST_SERIES[i].get());
    if (buf.length > HIST_MAX) buf.shift();
  }
}
...
const graph = {
  srcPorts: {}, instPorts: {}, instNodes: {}, srcVals: {},
...
function buildGraph() {
  ...
  const NODE_W = 118, ROW = 32, PAD = 14;
  const H = Math.max(srcKeys.length, INSTRUMENTS.length) * ROW + PAD * 2;
  el.style.height = `${H}px`;
  const svg = document.getElementById("graphSvg");

  const makeNode = (cls, label, x, y, portCls) => {
    const node = document.createElement("div");
    node.className = `gnode ${cls}`;
    node.style.left = `${x}px`;
    node.style.top = `${y}px`;
    node.style.width = `${NODE_W}px`;
    node.innerHTML = `<span class="glabel">${label}</span><span class="gval"></span>`;
  ...
  // Source nodes (left) with output ports.
  srcKeys.forEach((key, i) => {
    const { node, port } = makeNode("gnode--src", PARAMS[key].label, PAD, PAD + i * ROW, "gport--out");
    ...
    node.querySelector(".gval").style.background = PARAMS[key].color;
  });

  // Instrument nodes (right) with input ports.
  const instX = () => el.clientWidth - NODE_W - PAD;
  const instGap = (H - PAD * 2) / INSTRUMENTS.length;
  INSTRUMENTS.forEach((inst, i) => {
    const y = PAD + i * instGap + (instGap - 30) / 2;
    const { node, port } = makeNode("gnode--inst", inst.label, instX(), y, "gport--in");
  ...
  const reflow = () => {
    const x = instX();
    for (const inst of INSTRUMENTS) graph.instNodes[inst.key].style.left = `${x}px`;
  };
...
function portCenter(port) {
  // Coordinates relative to the graph container (offsetParent = node = positioned).
...
function refreshConnectionStyles() {
  const usedSrc = new Set(Object.values(connections).filter(Boolean));
...
function onGraphPointerDown(e) {
  ...
  // Click on a cable hit-area removes that connection.
  const hit = e.target.closest(".cable-hit");
  if (hit) disconnect(hit.dataset.inst);
}
...
function updateInstruments() {
  const svg = document.getElementById("graphSvg");
  for (const inst of INSTRUMENTS) {
    const src = connections[inst.key];
    const v = src ? paramValue(src) : 0;
...
async function init() {
  ...
  els.logToggle.addEventListener("click", () => {
    const hidden = els.log.classList.toggle("log--hidden");
    els.logToggle.textContent = hidden ? "show" : "hide";
  });
